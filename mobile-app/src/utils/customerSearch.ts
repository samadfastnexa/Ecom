import type { AdminCustomer } from '../services/adminService';

/**
 * Client-side customer matching, kept deliberately in step with the server's
 * `_customer_search_q` in accounts/views.py and with the web admin's
 * features/customers/search.ts.
 *
 * The list is fetched whole and filtered here so typing stays instant, but the
 * *rules* must not drift: an admin searching the same words on the phone and in
 * the web panel should find the same people.
 */
...
const haystack = (c: AdminCustomer): string[] => [
  c.name,
  c.username,
  c.address ?? '',
  c.house_number ?? '',
...
const matchesTerm = (c: AdminCustomer, term: string): boolean => {
  const lower = term.toLowerCase();
  if (haystack(c).some(field => field.toLowerCase().includes(lower))) return true;

  // Compare phones digit-for-digit so "03001234" finds "0300-1234567". Below
  // three digits this matches almost everyone, so it is not worth doing.
 ...
 */
export const matchesCustomerSearch = (c: AdminCustomer, query: string): boolean => {
